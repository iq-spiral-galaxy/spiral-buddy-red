...
function expectedUpdateAssetName(version) {
  ...
  if (process.platform === "win32") {
    return `Spiral.Buddy.Red.Setup.${version}.exe`;
  }
  return null;
}
...
function macUpdatePaths(version) {
  return {
    targetApp: "/Applications/Spiral Buddy Red.app",
    stagedApp: `/Applications/.Spiral Buddy Red.update-${version}.app`,
    backupApp: `/Applications/.Spiral Buddy Red.backup-${version}.app`,
    executableName: "Spiral Buddy Red",
    volumeAppName: "Spiral Buddy Red.app",
  };
}
...
function cleanupSuccessfulUpdateBackup(version) {
  ...
  const backupPath = path.join(
    "/Applications",
    `.Spiral Buddy Red.backup-${version}.app`,
  );
  try {
...
async function installUpdateMacOS(version, logPath, asset) {
  ...
  const dmgPath = path.join(
    os.tmpdir(),
    `spiral-buddy-red-${version}-${process.arch}.dmg`,
  );
  const download = await downloadUpdateAsset({
    asset,
    dest: dmgPath,

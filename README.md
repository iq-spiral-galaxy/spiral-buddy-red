...
- 🧠 **나선형 복습** — 같은 개념을 **depth 1**(직관·정의·정리 진술) → **depth 2**(핵심 정리 완전 증명) → **depth 3**(반례·변형·다른 레이어와 연결)로 반복하며 누적
...
### 💬 버디와의 Socratic 학습 세션
- **증명 중심** — "Prove, don't memorize": 정의 → 직관 → 정리 → 증명. 가정을 명시하고, 증명은 통째로 던지지 않고 다음 단계를 학습자가 먼저 시도
- **depth별 학습 의미** — depth 1: 직관·정의·정리 진술 / depth 2: 핵심 정리 완전 증명 (학습자 주도) / depth 3: 가정 무너뜨리기·반례·ML/DL/RL 응용 연결
- 이전 노트가 자동으로 새 세션 컨텍스트에 포함
- **스트리밍 응답** — 실시간 토큰 단위 표시
- **모델 선택** — Sonnet 4.6 (기본·추천) / Opus / Haiku 등. 완전 증명을 따라가는 depth 2~3 세션엔 Opus 권장
- **세션 Pause / Resume** — 일시정지 후 사이드바 PAUSED 섹션에서 멀티 세션 관리, 클릭으로 컨텍스트 유지하며 재개

### 🧮 수식이 깨지지 않는 채팅
- **KaTeX 렌더링** — 채팅·Look-up·노트 미리보기에서 `$...$` / `$$...$$` LaTeX가 그대로 수식으로 렌더링 (한국어 조사 밀착 `$\mathbb{R}^3$의`, `aligned`/`pmatrix` 환경 포함)
- **수식 클릭 → LaTeX 복사** — 렌더링된 수식을 클릭하면 원본 LaTeX가 구분자 포함으로 클립보드에 복사 → 노트·Obsidian·채팅 입력에 바로 붙여넣기
- **수식 입력 도우미** — LaTeX를 몰라도 입력창 아래 기호 팔레트(12개 카테고리·236개)에서 클릭만으로 행렬·적분·∑·전치·집합기호 등 입력. 빈칸은 **Tab으로 순회**, 자주 쓰는 기호는 자동으로 앞에. `$`를 치면 **라이브 미리보기**로 보내기 전에 수식 확인. 단축키 `⌘\`로 팔레트 토글
- **프롬프트 수준 강제** — 버디가 항상 KaTeX 호환 표기로 답하도록 세션·Look-up·노트 생성 프롬프트 전체에 수식 규칙 주입
- **렌더러 로컬 동봉** — KaTeX/marked를 CDN이 아닌 앱에 내장 — 네트워크 상태와 무관하게 UI와 수식 렌더링 동작
- 저장된 노트는 Obsidian MathJax와 같은 구분자라 vault에서도 동일하게 렌더링
...
### 🎯 깊이 있는 학습 도구
- **Quiz 단계별 난이도** — 누를수록 어려워짐 (개념 확인 → 적용 → 함정·반례 → 종합 문제: 증명 스케치 또는 ML 시스템 시나리오)
...
3. *(선택)* **트랙 프리셋 카드 클릭** — 🧮 수학 코어부터 🧭 전체 딥다이브까지 8종 중 선택 → iq-ai-lab 레포 자동 다운로드 (이미 받은 레포는 skip)

### 3. 학습 시작

좌측 사이드바에서 챕터 선택 → 버디와 대화 → `End & Save` 클릭 → 노트 보관함에 자동 생성. 트랙은 나중에 **설정 > iq-ai-lab 학습 자료**에서 도메인별로 더 받거나 뺄 수 있습니다.

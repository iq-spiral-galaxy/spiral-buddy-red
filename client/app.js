...
// MLOps 등)를 보존하는 prettyName 규칙이 더 풍부하므로 같은 표시 전용
// 계약을 Red formatter에 위임한다. id·경로·API 값에는 적용하지 않는다.
...
function renderDomainNode(domName, domEntry, catMeta) {
...
function renderRoadmapSelector() {
  ...
  els.roadmapCurrent.innerHTML = `
    <div class="roadmap-current-inner">
      <span class="roadmap-name">${activeIcon}<span class="roadmap-name-text">${escapeHtml(active ? prettyName(activeName) : activeName)}</span></span>
...
function roadmapItemHtml(r) {
  ...
  const lastDateLabel =
    lastDate === "—" ? "최근 학습 기록 없음" : `마지막 학습 ${lastDate}`;
  const displayName = cleanUiLabel(r.name) || String(r.name ?? "");
  return `
      ...
      return `
        <div class="ws-row ${isActive ? "active" : ""}">
          <div class="ws-row-main">
            ...
            <div class="ws-row-vaultsub">노트: <code>vault/${escapeHtml(w.vaultSubDir ?? "spiral-buddy-red")}/</code></div>
          </div>
...
function openRemoveWorkspaceModal(ws) {
  ...
  document.getElementById("remove-ws-notes-path").innerHTML = `<code>${escapeHtml(_settingsCache.vaultPath ?? "")}/${escapeHtml(ws.vaultSubDir ?? "spiral-buddy-red")}</code> 이 옆 폴더로 이동(보관)됩니다.`;
...
function openDeletePopover(anchorEl, target) {
  ...
  const hint = `<div class="delete-popover-hint">현재 워크스페이스의 .trash/로 이동 — 복구 가능</div>`;
  ...
  if (state.session) {
    ...
    els.topbar.innerHTML = `
      <button class="current-chapter-jump" type="button" title="사이드바에서 현재 학습 위치 보기" aria-label="${escapeAttr(chapterTitle)} — 사이드바에서 현재 학습 위치 보기" aria-controls="roadmap-list" aria-expanded="${String(!els.roadmapList.classList.contains("hidden"))}">
        ...
        <strong class="topbar-chapter-title" title="${escapeAttr(chapterTitle)}${rmName ? ` — ${escapeAttr(rmName)}` : ""}">${escapeHtml(chapterTitle)}</strong>

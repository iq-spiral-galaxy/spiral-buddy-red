...
function registerAiRoutes(app: Hono, config: Config, client: ClaudeClient) {
  ...
  app.post("/chapter-context", async (c) => {
    ...
    const systemPrompt =
      ...
      "- Buddy가 본문에 없는 detail을 말한 것 같으면 주의 문구로 알린다: " +
      "'주의: 본문에선 이 부분 직접 안 다룸 — Buddy가 일반 지식 기반으로 말한 것 같음'.\n" +
...
function registerSessionRoutes(app: Hono, config: Config, client: ClaudeClient) {
  ...
  app.post("/session/start", async (c) => {
    ...
    // canonical local id는 root 전체(사용자 환경 632 roadmaps)를 다시 찾지 않고
    // 해당 디렉터리 하나만 realpath/readdir로 검증한다. fuzzy/curated 입력만
    // 기존 discover fallback을 탄다.
    ...
    // 일반 학습은 선택한 파일 하나만 읽으면 된다. 검증 보강 세션만 sibling
    // chapter 매칭이 필요하므로 전체 목록을 유지한다. vault scan은 독립 I/O라
    // chapter read와 병렬로 시작한다.
    ...
    if (body.verificationAttemptId) {
      // Attempts remain on disk even if a user later deletes the d1 note. Do not
      // let that historical JSON become a back door into a depth-2 session.
    ...
    try {
      // createSession 내부에서 검사와 등록을 동기적으로 한 번에 수행한다.
      // 따라서 같은 attempt의 동시 요청 중 하나만 세션을 점유할 수 있고,
      // 클라이언트에서 pause한 세션도 취소/완료 전까지 예약을 유지한다.
    ...
    return streamText(c, async (stream) => {
      let deliveredText = "";
      try {
        // 응답 metadata/session id를 첫 AI token보다 먼저 브라우저에 확실히
        // 전달한다. leading newline은 Markdown 화면과 저장 transcript에는 보이지 않는다.
        await stream.write("\n");

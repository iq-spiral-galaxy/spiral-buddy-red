...
function canonicalRelativeSegments(value: string): string[] | null {
...
function hasIgnoredPathSegment(segments: string[]): boolean {
...
export async function resolveLocalRoadmapExact(
  ...
  try {
    ...
    const isRootRoadmap = roadmapId === rootId;
    // discoverRoadmaps는 root 아래 MAX_DEPTH까지만 내려가며 숨김 폴더와
    // node_modules는 방문하지 않는다. exact fast path도 같은 후보 집합만 허용한다.
    if (
      !isRootRoadmap &&
      (segments.length > MAX_DEPTH || hasIgnoredPathSegment(segments))
    ) {
      return null;
    }
    const candidate =
...
export async function loadRoadmapChapter(
  ...
  if (!segments) return null;
  if (
    hasIgnoredPathSegment(segments) ||

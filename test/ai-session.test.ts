...
//
// RED note: this color writes notes under <vault>/spiral-buddy-red/ (SPIRAL_DIR
// default), not <vault>/spiral-buddy/. The end-of-session assertion below uses
// the red literal accordingly.
// ─────────────────────────────────────────────────────────────────────────
...
describe("POST /session/:id/end", () => {
  ...
  test("writes a .md note to the vault, emits SSE stages, and closes the session", async () => {
    ...
    try {
      ...
      // a note file actually landed under <vault>/spiral-buddy-red/ (RED's
      // SPIRAL_DIR default — Blue uses "spiral-buddy").
      const notes = await listSpiralNotes(v);
      assert.equal(notes.length, 1);
      const spiralDir = path.join(v, "spiral-buddy-red");

...
describe("vault: listSpiralNotes", () => {
  ...
  test("parses new-schema note, ignores _index.md, sorts newest-first", async () => {
    ...
    const spiralRoot = path.join(vault, "spiral-buddy-red");
    await writeRaw(
      spiralRoot,
      "older d1.md",
      [
  ...
  test("writeNewNote writes through an already-loaded note cache", async () => {
    ...
    await fs.rename(
      path.join(vault, "spiral-buddy-red"),
      path.join(vault, "spiral-buddy-red-moved"),
    );
    const notes = await listSpiralNotes(vault);
    assert.equal(notes.length, 1);
...
describe("roadmap: discoverRoadmaps + loadRoadmapChapters fixture", () => {
  ...
  test("exact roadmap/chapter loaders block traversal, ignored paths, excess depth and outside symlinks", async () => {
    const root = await mkTmp("rmsafe");
    const repo = path.join(root, "safe-rm");
    const outside = await mkTmp("rmoutside");
    await fs.mkdir(path.join(repo, ".draft"), { recursive: true });
    ...
    await fs.writeFile(path.join(repo, ".draft", "hidden.md"), "# hidden\n");
    await fs.writeFile(path.join(repo, "node_modules", "hidden.md"), "# hidden\n");
    ...
    const hiddenRoadmap = path.join(root, ".hidden-rm");
    await fs.mkdir(hiddenRoadmap);
    await fs.writeFile(path.join(hiddenRoadmap, "01-a.md"), "# A\n");
    await fs.writeFile(path.join(hiddenRoadmap, "02-b.md"), "# B\n");
    const tooDeepId = "one/two/three/four/five/six/seven";
    ...
    assert.equal(await resolveLocalRoadmapExact(root, "escape"), null);
    assert.equal(await resolveLocalRoadmapExact(root, ".hidden-rm"), null);
    assert.equal(
      await resolveLocalRoadmapExact(root, "node_modules/inner"),
      null,
    );
    ...
    assert.equal(await loadRoadmapChapter(roadmap!, ".draft/hidden.md"), null);
    assert.equal(
      await loadRoadmapChapter(roadmap!, "node_modules/hidden.md"),
      null,
    );
    assert.equal(await loadRoadmapChapter(roadmap!, "outside.md"), null);
  });

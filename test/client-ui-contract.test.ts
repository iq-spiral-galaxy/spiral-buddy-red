...
describe("client UI contracts", () => {
  ...
  test("trash copy follows the active workspace and Red's default vault folder", () => {
    ...
    assert.match(
      mcpSource,
      /vault의 \$\{vaultSubDir\}\/\.trash\/로 이동합니다/,
    );
    assert.ok(
      (mcpSource.match(/path\.join\(vaultPath, vaultSubDir,/g) ?? []).length >= 2,
      "MCP note reads and related-note paths must follow the explicit workspace subdir",
    );
    assert.doesNotMatch(
      mcpSource,
      /path\.join\(vaultPath, "spiral-buddy-red",/,
    );
    assert.doesNotMatch(mcpSource, /vault의 spiral-buddy\/\.trash\//);
  });

  test("shared learning UI has a Red-aware repository display-name helper", () => {
    const helperAt = app.indexOf("function displayRepoName(name)");
    const firstCallAt = app.indexOf("displayRepoName(activeRoadmap.name)");

    assert.ok(helperAt >= 0, "displayRepoName must be defined in app.js");
    assert.ok(firstCallAt >= 0, "the shared learning UI should use displayRepoName");
    assert.ok(
      helperAt < firstCallAt,
      "the display-name helper should be declared before its first shared-UI call",
    );
    assert.match(
      app.slice(helperAt, app.indexOf("function displayWorkspaceName", helperAt)),
  ...
  test("the learning home appears before slow secondary requests and survives start failures", () => {
    ...
    assert.match(baseCss, /\.message\.assistant\.is-waiting > \.content::before/);
  ...
  test("light composer is one surface and desktop junctions are rounded", () => {
    ...
    assert.match(html, /red-brand\.css\?v=0\.7\.1/);

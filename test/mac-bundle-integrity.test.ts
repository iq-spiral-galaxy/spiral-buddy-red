...
function makeFixture(root: string, appName = "Spiral Buddy Red.app") {
  ...
  fs.writeFileSync(path.join(macOS, "Spiral Buddy Red"), "fixture", { mode: 0o755 });
...
describe("macOS update bundle integrity", () => {
  ...
  test("accepts a complete app whose framework links are relative and contained", () => {
    ...
    assert.equal(isValidMacAppBundle(appPath, "Spiral Buddy Red"), true);
  });

  test("rejects absolute, broken and bundle-escaping framework links", () => {
    for (const target of [
      "/Volumes/Detached DMG/Spiral Buddy Red.app/Contents/Frameworks/Electron Framework",
      ...
      assert.equal(isValidMacAppBundle(appPath, "Spiral Buddy Red"), false, target);
    }
  });
  ...
  test("verbatim staging remains launchable after the source bundle disappears", async () => {
    ...
    assert.equal(isValidMacAppBundle(stagedApp, "Spiral Buddy Red"), true);
    assert.equal(
      fs.readlinkSync(
        path.join(
          stagedApp,

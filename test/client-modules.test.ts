...
describe("util.stripDisplayOrderPrefix", () => {
  test("removes display-only numeric and chapter prefixes", () => {
    assert.equal(stripDisplayOrderPrefix("01-object-model"), "object-model");
    assert.equal(stripDisplayOrderPrefix("02. References"), "References");
    assert.equal(stripDisplayOrderPrefix("03_index"), "index");
    assert.equal(stripDisplayOrderPrefix("Chapter01 - Intro"), "Intro");
    assert.equal(stripDisplayOrderPrefix("Ch2: Cache"), "Cache");
  });

  test("preserves words and four-digit titles that only look like prefixes", () => {
    assert.equal(stripDisplayOrderPrefix("Chrome"), "Chrome");
    assert.equal(stripDisplayOrderPrefix("1984-orwell"), "1984-orwell");
    assert.equal(stripDisplayOrderPrefix("12 Factor App"), "12 Factor App");
    assert.equal(displayChapterTitle("01 Object Model"), "Object Model");
  });
});
...
describe("icons.svgIcon (via groupIconHtml)", () => {
  test("known icon embeds its body and wraps in <svg>…</svg>", () => {
    const out = svgIconViaGroup("bolt");
  ...
  test("unknown icon falls back to folder body", () => {
    const out = svgIconViaGroup("totally-not-a-real-icon");
    const folder = svgIconViaGroup("folder");
  ...
  test("includes standard svg attributes (viewBox, stroke, aria-hidden)", () => {
    const out = svgIconViaGroup("leaf");

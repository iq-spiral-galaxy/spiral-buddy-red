...
describe("getOrgCategories / getOrgDomains (real data)", () => {
  ...
  // RED data (iq-ai-lab): first domain by order is Mathematics (order 1).
  test("iq-ai-lab domains are sorted by order ascending", async () => {
    const domains = await getOrgDomains("iq-ai-lab");
    assert.ok(domains && domains.length > 0);
    const orders = domains!.map((d) => d.order);
    const sorted = [...orders].sort((a, b) => a - b);
    assert.deepEqual(orders, sorted);
    // Mathematics (order 1) comes first per the data file
    assert.equal(domains![0]!.name, "Mathematics");
  });
  ...
  test("getOrgCategories flattens domains in domain-order", async () => {
    ...
    assert.equal(cats![0]!.name, "Mathematics");
  });
});

describe("findDomainForCategory (real data)", () => {
  test("finds the owning domain by exact category name", async () => {
    const d = await findDomainForCategory("iq-ai-lab", "Mathematics");
    assert.ok(d);
    assert.equal(d!.id, "mathematics");
  });

  test("category name matching is normalized (spaces/&/case)", async () => {
    // "CV & 3D" should match a normalized variant (drops spaces/&/case)
    const d = await findDomainForCategory("iq-ai-lab", "cv&3d");
    assert.ok(d);
    assert.equal(d!.name, "CV & 3D");
  });

  test("unknown category → null", async () => {
    assert.equal(await findDomainForCategory("iq-ai-lab", "Nonexistent Cat"), null);
  });

  test("unknown org → null", async () => {
    assert.equal(await findDomainForCategory("no-org", "Mathematics"), null);
  });
});
...
describe("groupReposByCategory", () => {
  ...
  test("known org: matched repos grouped by category in defined order", async () => {
    // RED (iq-ai-lab): linear-algebra-deep-dive is the FIRST repo in Mathematics,
    // probability-theory-deep-dive is the SECOND.
    const repos = [
      { name: "probability-theory-deep-dive" },
      { name: "linear-algebra-deep-dive" },
    ];
    const groups = await groupReposByCategory("iq-ai-lab", repos);
    const found = groups.find((g) => g.category.name === "Mathematics");
    assert.ok(found, "Mathematics group should exist");
    // ordering follows cat.repos index, not input order:
    // linear-algebra-deep-dive (idx0) before probability-theory-deep-dive (idx1)
  ...
  test("only categories with matched repos are emitted (no empty groups)", async () => {
    const repos = [{ name: "linear-algebra-deep-dive" }];
    const groups = await groupReposByCategory("iq-ai-lab", repos);
    // Just the one Mathematics group, no empties from other categories
    assert.equal(groups.length, 1);
    assert.equal(groups[0]!.category.name, "Mathematics");
    assert.ok(groups.every((g) => g.repos.length > 0));
  });

  test("empty repo list on known org → no groups", async () => {
    const groups = await groupReposByCategory("iq-ai-lab", []);
...
describe("categorizeLocalRoadmap", () => {
  test("empty roadmapId → Topics (UNCATEGORIZED)", async () => {
    const c = await categorizeLocalRoadmap("iq-ai-lab", "");
  ...
  test("first segment matches a category name → that category def", async () => {
    const c = await categorizeLocalRoadmap("iq-ai-lab", "Mathematics/anything/01-x");
    assert.equal(c.name, "Mathematics");
  ...
  test("first segment matches a repo name → owning category def", async () => {
    // linear-algebra-deep-dive lives under Mathematics
    const c = await categorizeLocalRoadmap(
      "iq-ai-lab",
      "linear-algebra-deep-dive/sub/01-x",
    );
    assert.equal(c.name, "Mathematics");
  });
  ...
  test("repo match honors -deep-dive optional suffix", async () => {
    const c = await categorizeLocalRoadmap(
      "iq-ai-lab",
      "linear-algebra/sub/01-x",
    );
    assert.equal(c.name, "Mathematics");
  });

  test("no match + 2+ segments → first segment as ad-hoc category (📁)", async () => {
    const c = await categorizeLocalRoadmap("iq-ai-lab", "mystery-topic/sub-x");
    assert.equal(c.name, "mystery-topic");
    assert.equal(c.emoji, "📁");
  });

  test("no match + single segment → Topics", async () => {
    const c = await categorizeLocalRoadmap("iq-ai-lab", "mystery-topic");
    assert.equal(c.name, "Topics");
  });
